...
from standalone_variations.rule_based_name_variations import generate_for_rules, RULE_GENERATORS
try:
    import jellyfish  # for jaro_winkler and metaphone/nysiis
except Exception:
    jellyfish = None
try:
    import Levenshtein  # for edit distance / ratio
except Exception:
    Levenshtein = None
...
class CountryManager:
    ...
    def _load_sanctioned_list(self):
        ...
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            json_path = os.path.join(current_dir, "..", "MIID", "validator", "Sanctioned_list.json")
            if os.path.exists(json_path):
                with open(json_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
    ...
    def _detect_script_from_country(self, country: str) -> ScriptType:
        ...
        # Arabic countries
        arabic_countries = ['syria', 'iraq', 'iran', 'afghanistan', 'sudan', 'lebanon',
                           'libya', 'somalia', 'yemen', 'algeria']
        ...
        # Cyrillic countries
        cyrillic_countries = ['russia', 'ukraine', 'belarus', 'bulgaria', 'armenia',
                             'crimea', 'donetsk', 'luhansk']
        if any(cyrillic in country_lower for cyrillic in cyrillic_countries):
            return ScriptType.CYRILLIC

        # Other scripts
        other_scripts_countries = ['myanmar', 'laos', 'nepal', 'vietnam', 'india',
                                 'thailand', 'israel', 'greece', 'georgia']
    ...
    def _add_latin_countries(self):
        ...
        latin_countries = [
            ...
            "Spain", "France", "Italy", "Portugal", "Romania", "Moldova",
            ...
            "Haiti", "Jamaica", "Trinidad and Tobago", "Barbados", "Guyana",
            "Suriname", "Belize", "Grenada", "Saint Lucia", "Saint Vincent and the Grenadines",
            "Antigua and Barbuda", "Dominica", "Saint Kitts and Nevis",
            # African Latin countries
            "Angola", "Mozambique", "Cape Verde", "Guinea-Bissau", "São Tomé and Príncipe",
            "Equatorial Guinea", "Chad", "Central African Republic", "Cameroon",
            "Ivory Coast", "Burkina Faso", "Mali", "Niger", "Senegal", "Guinea",
            "Sierra Leone", "Liberia", "Ghana", "Togo", "Benin", "Nigeria",
            "Gabon", "Republic of the Congo", "Democratic Republic of the Congo",
            "Rwanda", "Burundi", "Madagascar", "Mauritius", "Seychelles",
            "Comoros", "Djibouti", "Eritrea", "Ethiopia", "Somalia", "Kenya",
            "Tanzania", "Uganda", "South Sudan", "Sudan", "Egypt", "Libya",
            "Tunisia", "Algeria", "Morocco", "Western Sahara", "Mauritania",
            "Mali", "Burkina Faso", "Niger", "Chad", "Central African Republic",
            "Cameroon", "Nigeria", "Benin", "Togo", "Ghana", "Burkina Faso",
            "Ivory Coast", "Liberia", "Sierra Leone", "Guinea", "Guinea-Bissau",
            "Senegal", "Gambia", "Mauritania", "Mali", "Burkina Faso", "Niger",
            "Chad", "Sudan", "South Sudan", "Ethiopia", "Eritrea", "Djibouti",
            ...
            "Cameroon", "Equatorial Guinea", "Gabon", "São Tomé and Príncipe",
            "Angola", "Zambia", "Malawi", "Mozambique", "Madagascar", "Mauritius",
            "Seychelles", "Comoros", "Mayotte", "Réunion", "Saint Helena",
            "Ascension Island", "Tristan da Cunha", "Bouvet Island", "South Georgia",
            "South Sandwich Islands", "Falkland Islands", "South Shetland Islands",
            "South Orkney Islands", "South Georgia and the South Sandwich Islands",
            # Other Latin countries
            "Philippines", "East Timor", "Vatican City", "San Marino", "Andorra",
            "Liechtenstein", "Monaco", "Luxembourg", "Belgium", "Netherlands",
            "Switzerland", "Austria", "Germany", "Denmark", "Sweden", "Norway",
            "Finland", "Iceland", "Ireland", "United Kingdom", "Malta", "Cyprus"
        ]
...
def detect_script(text: str) -> ScriptType:
    ...
    ranges = [
        ...
        (ScriptType.CYRILLIC, [(0x0400, 0x04FF), (0x0500, 0x052F), (0x2DE0, 0x2DFF), (0xA640, 0xA69F)]),
...
def _generate_latin_variations(name: str, count: int, country_info: Optional[CountryInfo] = None) -> List[str]:
...
def _generate_quality_variations(name: str, count: int) -> List[str]:
    ...
    middle_names = ["James", "Michael", "David", "Robert", "William", "Richard", "Thomas", "Christopher"]
...
def _select_variations_by_weights(variations: List[str], original: str,
...
def generate_name_variations(
    ...
    if script == ScriptType.LATIN:
        base_variations = _generate_latin_variations(name, expected_count * 10, country_info)
    ...
    if len(unique_variations) < expected_count * 3:
        ...
        unique_variations.extend([v for v in additional_variations if v not in unique_variations and v != name])
    ...
    if rule_count > 0 and selected_rules:
        try:
            from standalone_variations.rule_based_name_variations import generate_for_rules
    ...
    if similarity_count > 0:
        similarity_variations = _select_variations_by_weights(
            unique_variations, name, phonetic_config, orthographic_config, similarity_count * 2  # Generate more than needed
        )
    ...
    if len(all_variations) < expected_count:
        ...
        additional_variations = [v for v in unique_variations if v not in all_variations][:additional_needed]
...
def predict_name_score(original: str, variation: str, name_weight: float = 0.3) -> float:
